...
require_once BASE_PATH.'/Lib/httpSqs.php';

cli_set_process_title("php httpServerHttpSqs.php: httpServerHttpSqs");  // 设置程序的进程名

class httpServerHttpSqs
{
    ...
    public function __construct()
    {
        ...
        $httpsqs_config = $allConfig['httpsqs'];

        $this->serv = new swoole_http_server($server_config['host'], $server_config['port']);
        $this->serv->set($server_config);

        $this->httpsqs = new httpsqs($httpsqs_config['host'],$httpsqs_config['port'],$httpsqs_config['auth']);
     ...
     * 异步任务处理中心
     * @param  [type] $serv    [服务]
     * @param  [type] $task_id [处理任务id]
     * @param  [type] $from_id [description]
     * @param  [type] $data    [传递的数据]
     * @return [type]          [返回给onFinish的值]
     */
    public function onTask($serv, $task_id, $from_id, $data)
    {
        $this->httpsqs->put('sw_chinaz_keyword_result',$data);
